...
const App = {
  ...
  initPresets() {
    ...
    // SerDes GT type selector (not a preset, but needs URL update)
    const serdesGtType = document.getElementById('serdes-gttype');
    if (serdesGtType) {
      serdesGtType.addEventListener('change', () => {
        this.updateUrl();
      });
    }
  },
...
const CopyResults = {
  ...
  formatSerdesResults() {
    ...
    const gtSelect = document.getElementById('serdes-gttype');
    text += '- **GT Type:** ' + gtSelect.options[gtSelect.selectedIndex].text + '\n\n';
